...
for ii = 1 : num_MC
    ...
    % Propagate dynamics 
    [tout_FDOC, xout_FDOC]   = ode45(@(t,x) ZermeloDynamics(t,x,t_FDOC,u_FDOC,p_ii), tspan, x0, options);
    [tout_BL, xout_BL]       = ode45(@(t,x) ZermeloDynamics(t,x,t_BL,u_BL,p_ii), tspan, x0, options);
...
plot(x_BL_nominal(:,1),x_BL_nominal(:,2),'k','LineWidth',1);
ylim([0 0.45]);
...
plot(x_FDOC_nominal(:,1),x_FDOC_nominal(:,2),'k','LineWidth',1);
ylim([0 0.45]);
