...
fdoc_LQR_plot_style = {'--r','LineWidth',1.5};
postProcess_FDOC_v2(feedbackLQR,fdoc_LQR_plot_style);

%% Solve Hinf feedback DOCP
feedbackHinf  = obtain_FDOC_solution_Hinf(C, IC, FC, LB, UB);

%% Plot desensitized state and control (Hinf feedback)
fdoc_Hinf_plot_style = {'k','LineWidth',1.5};
postProcess_FDOC_v2(feedbackHinf,fdoc_Hinf_plot_style);

%% Solve mixed H2 / Hinf feedback DOCP
feedbackMixed = obtain_FDOC_solution_mixedDG(C, IC, FC, LB, UB);

%% Plot desensitized state and control (mixed H2 / Hinf feedback)
fdoc_mixed_plot_style = {'b','LineWidth',1.5};
...
fdoc_Hinf_scatter_style  = {'ks','LineWidth',1};
fdoc_mixed_scatter_style = {'b+','LineWidth',2};
fdoc_LQR_scatter_style   = {'rx','LineWidth',1};
...
t_FDOC_Hinf  = feedbackHinf.phase.time;
u_FDOC_Hinf  = feedbackHinf.phase.control;

% Get optimal H2 / Hinf CL desensitized control and times
...
xhist_FDOC_Hinf  = zeros(M, 2, num_MC);
...
for ii = 1 : num_MC
    ...
    % Propagate dynamics 
    [tout_FDOC_Hinf, xout_FDOC_Hinf]   = ode45(@(t,x) ZermeloFeedbackDynamics_v2(t,x,feedbackHinf,C,p_ii,1), tspan, x0, options);
    ...
    % Store MC trajectory
    xhist_FDOC_Hinf(:, :, ii)  = xout_FDOC_Hinf;
...
x_FDOC_Hinf           = feedbackHinf.phase.state;
...
for k = 1 : M
    ...
    full_state_FDOC_Hinf_k     = interp1(t_FDOC_Hinf, x_FDOC_Hinf, tk);
    x_FDOC_Hinf_nominal(k, :)  = full_state_FDOC_Hinf_k(1:2);
...
title('Closed Loop Mixed $\mathcal{H}_{2}/\mathcal{H}_{\infty}$ Feedback Desensitized','Interpreter','latex','FontSize',12);
xlabel('Downstream','FontSize',15);
ylabel('Upstream','FontSize',15);
set(gca,'FontSize',15);
subplot(3,2,5); hold on; grid on;
title('Closed Loop $\mathcal{H}_{\infty}$ Feedback Desensitized','Interpreter','latex','FontSize',12);
xlabel('Downstream','FontSize',15);
ylabel('Upstream','FontSize',15);
set(gca,'FontSize',15);
...
subplot(3,2,5);
for ii = 1 : num_MC
    plot(xhist_FDOC_Hinf(:,1,ii),xhist_FDOC_Hinf(:,2,ii),'color',grey);
end
plot(x_FDOC_Hinf_nominal(:,1),x_FDOC_Hinf_nominal(:,2),'k','LineWidth',2);

% Compute errors between MC and nominal
error_FDOC_Hinf        = zeros(M, 2, num_MC);
error_FDOC_mixed       = zeros(M, 2, num_MC);
error_FDOC_LQR         = zeros(M, 2, num_MC);
error_DOC              = zeros(M, 2, num_MC);
error_BL               = zeros(M, 2, num_MC);
final_error_FDOC_Hinf  = zeros(num_MC, 2);
...
for ii = 1 : num_MC
    ...
    error_FDOC_Hinf(:, :, ii)     = x_FDOC_Hinf_nominal - xhist_FDOC_Hinf(:, :, ii);
    ...
    final_error_FDOC_Hinf(ii, :)  = error_FDOC_Hinf(end, :, ii); 
...
covariance_FDOC_Hinf  = cov(final_error_FDOC_Hinf);
...
mean_FDOC_Hinf        = mean(final_error_FDOC_Hinf)';
mean_FDOC_mixed       = mean(final_error_FDOC_mixed)';
...
length_FDOC_Hinf  = sqrt(-2 * log(1 - conf)) * sqrt(max(eig(covariance_FDOC_Hinf)));
...
set(gca,'FontSize',15);
scatter(final_error_FDOC_Hinf(:,1),  final_error_FDOC_Hinf(:,2),  fdoc_Hinf_scatter_style{:});
...
conf_ellipse(mean_FDOC_Hinf,   covariance_FDOC_Hinf,  Npl, conf, 1, fdoc_Hinf_plot_style{:});
conf_ellipse(mean_FDOC_mixed,  covariance_FDOC_mixed, Npl, conf, 1, fdoc_mixed_plot_style{:});
...
legend({'$\mathcal{H}_{\infty}$ Feedback','Mixed $\mathcal{H}_{2}/\mathcal{H}_{\infty}$','LQR Feedback','Open Loop','Baseline'},...
